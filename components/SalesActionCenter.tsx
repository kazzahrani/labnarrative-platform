...
export default function SalesActionCenter() {
  ...
  const load = useCallback(async (activeSession: Session) => {
    ...
    try {
      ...
      const [prospectResult, siteResult, messageResult, linkedinResult] = await Promise.all([
  ...
  const stats = useMemo(() => {
    ...
    const engaged = sites.filter((row) => ENGAGED.has(row.outreach_status)).length;
    const linkedinBacklog = linkedin.filter((row) => row.status === "not_contacted").length;
    return { initialSent: initialSent.length, sent24h, confirmedDelivered, followupsDue, deliveryIssues, engaged, linkedinBacklog };
  }, [linkedin, messages, sites]);
      ...
      <div className={styles.panel}>
        <header className={styles.header}>
          <div>
            ...
            <p>Prioritize conversations and exceptions. The detailed analytics and LinkedIn workflow remain below.</p>
          </div>
        ...
        <div className={styles.metrics}>
          ...
          <article><span>Engaged leads</span><strong>{stats.engaged}</strong><small>Replied, interested, meeting, proposal or client</small></article>
