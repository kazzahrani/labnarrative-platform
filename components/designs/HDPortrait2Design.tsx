...
export default function HDPortrait2Design(props: Props) {
  ...
  if (props.route.section !== "home") return <CiribilliNaritaDesign {...props} />;
  ...
  return (
    <div className="p2" style={vars}>
      ...
      <style jsx global>{`
        ...
        .p2-nav{min-height:88px;padding:0 clamp(24px,4.5vw,72px);display:flex;align-items:center;justify-content:space-between;gap:28px;background:var(--p2-bg,#f6f0e9);border-bottom:1px solid color-mix(in srgb,var(--p2-accent) 18%,transparent)}.p2-brand{flex:0 0 auto;color:var(--p2-accent);font-size:14px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.p2-nav nav{display:flex;align-items:center;gap:clamp(14px,2vw,29px);overflow-x:auto;scrollbar-width:none}.p2-nav nav::-webkit-scrollbar{display:none}.p2-nav nav a{position:relative;flex:0 0 auto;padding:10px 0;font-size:10px;font-weight:600;color:color-mix(in srgb,var(--p2-ink) 72%,transparent)}.p2-nav nav a:hover,.p2-nav nav a.active{color:var(--p2-accent)}.p2-nav nav a.active:after{content:"";position:absolute;left:0;right:0;bottom:3px;height:1px;background:var(--p2-accent)}
        ...
        .p2-footer{padding:52px clamp(24px,5vw,78px);display:grid;grid-template-columns:1.2fr .8fr 1fr;gap:40px;background:color-mix(in srgb,var(--p2-accent) 74%,#160f12);color:#fff}.p2-footer>div{display:flex;flex-direction:column;gap:7px}.p2-footer strong{margin-bottom:5px;font-family:Georgia,serif;font-size:20px;font-weight:400}.p2-footer a,.p2-footer span{color:rgba(255,255,255,.68);font-size:10px;line-height:1.55}.p2-footer a:hover{color:#fff}
