...
export async function authorizeTenderAutomation(request: Request, supabase: SupabaseClient) {
  const token = bearerToken(request);
  if (!token) return false;

  // Keep Vercel CRON_SECRET compatibility if it is configured later.
  const envSecret = process.env.CRON_SECRET?.trim();
  if (envSecret && safeEqual(sha256(token), sha256(envSecret))) return true;

  const { data, error } = await supabase
    .from("tender_automation_config")
