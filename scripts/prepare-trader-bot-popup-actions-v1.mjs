...
for(const required of[marker,'trader-account-control-v2','>Copy bot</button>','>Delete bot</button>','action: "copy_bot"','action: "delete_bot"']){
  if(!source.includes(required))throw new Error(`Bot popup action output missing ${required}`);
}
fs.writeFileSync(target,source);
console.log("Prepared bot popup Copy bot and safe Delete bot actions.");
