...
if (!configurator.includes(upperMarker)) {
  const viewAt = configurator.indexOf('if(mode==="view")return');
  const marketHeadingAt = configurator.indexOf("<h3>Market</h3>", viewAt);
  const marketStart = marketHeadingAt >= 0 ? configurator.lastIndexOf('<section className={cfg.card}>', marketHeadingAt) : -1;
  const marketEndStart = marketHeadingAt >= 0 ? configurator.indexOf("</section>", marketHeadingAt) : -1;
  if (viewAt < 0 || marketHeadingAt < 0 || marketStart < viewAt || marketEndStart < 0) throw new Error("DCA configurator final Market card boundaries missing");
  const marketEnd = marketEndStart + "</section>".length;
  const marketCard = configurator.slice(marketStart, marketEnd);
  ...
  configurator = configurator.slice(0, marketStart) + upperRow + configurator.slice(marketEnd);
}
for (const marker of [insightImport,upperMarker,"accountId={accountId}","botId={botId!}","outcomeOnly"]) if (!configurator.includes(marker)) throw new Error(`DCA configurator upper-right Outcome Mix missing ${marker}`);
