...
// redundant headline win-rate number, and express the outcome breakdown as shares.
...
if (!piesCss.includes("OUTCOME_ONLY_POPUP_V1")) {
...
// DCA Automations read-only popup. Inject after every earlier structural transform
// so the compact DCA detail shown from Automations retains the Outcome Mix card.
...
for (const marker of [importLine, dcaPieMarker, "botId={bot.id}", "outcomeOnly"]) {
  if (!shell.includes(marker)) throw new Error(`DCA Outcome Mix popup output missing ${marker}`);
}
