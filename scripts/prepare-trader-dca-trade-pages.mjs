...
// DCA trade ledger is separate from bot definitions and SmartTrades.
...
if (!source.includes('type DcaTrade = {')) {
  const anchor = 'type DcaBot = {';
  const start = source.indexOf(anchor);
  const end = source.indexOf('\n};', start) + 3;
  const dcaTradeType = `\ntype DcaTrade = {\n  id: string;\n  botId: string;\n  botName: string;\n  pair: string;\n  entryPrice: number;\n  averagePrice: number;\n  quantity: number;\n  invested: number;\n  averagingFilled: number;\n  maxAveraging: number;\n  status: \"Active\" | \"Closed\";\n  createdAt: string;\n  closedAt?: string;\n  realizedPnl?: number;\n};\n`;
  if (start >= 0 && end > start) source = source.slice(0, end) + dcaTradeType + source.slice(end);
}
...
source = source.replace(
  '<strong className={styles.greenText}>$0.00</strong><small>Paper mode</small>',
  '<strong className={dcaRealized >= 0 ? styles.greenText : styles.redText}>{compactMoney(dcaRealized)}</strong><small>Paper mode</small>'
);

// Add a 3Commas-style DCA submenu beneath DCA Bot.
