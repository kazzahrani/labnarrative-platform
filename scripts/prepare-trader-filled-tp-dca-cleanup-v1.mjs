...
const replaceRequired = (source, before, after, label) => {
  if (!source.includes(before)) throw new Error(`Filled TP/DCA cleanup: missing ${label}`);
  return source.replace(before, after);
};

// Trade edit: filled TPs are immutable, TP-only changes do not touch DCA/Binance orders.
let actions = fs.readFileSync(actionsPath, "utf8");
actions = replaceRequired(actions,
...
bar = replaceRequired(bar,
  ...
  "bar filled TP markers",
);
bar = bar.replace(
  `className={\`${styles.marker} ${styles[marker.kind]}\`}`,
  `className={\`${styles.marker} ${styles[marker.kind]} \${marker.filled ? styles.filledTp : ""}\`}`,
);
fs.writeFileSync(barPath, bar);
let barCss = fs.readFileSync(barCssPath, "utf8");
if (!barCss.includes("filled-tp-marker-v1")) barCss += `\n/* filled-tp-marker-v1 */\n.marker.filledTp:before{background:#2b6650!important}.marker.filledTp{color:#547766!important}.marker.filledTp b{color:#668978!important}.marker.filledTp span{opacity:.68}\n`;
fs.writeFileSync(barCssPath, barCss);

// Chart: remove the custom drawing toolbar only. Keep compact axis/indicator cleanup from the preceding patch.
let chart = fs.readFileSync(chartPath, "utf8");
chart = chart.replace(/\nimport ChartDrawingTools from "\.\/ChartDrawingTools";[^\n]*/g, "");
chart = chart.replace(/<ChartDrawingTools chartRef=\{chartRef\} seriesRef=\{candleSeriesRef\} candles=\{candles\} memoryKey=\{`\$\{accountId\}\|\$\{tradeId\}\|\$\{interval\}`\}\/>/g, "");
chart = chart.replace(
...
chart = chart.replace(
  ...
  `    tpPrices.forEach((order, index) => { const target = trade.takeProfitTargets?.[index]; const filled = Boolean(target?.filled || (trade.takeProfitFilled ?? []).includes(index + 1)); if (order.price) candleSeries.createPriceLine({ price: order.price, color: filled ? "#2c6b53" : "#57c99c", lineWidth: 1, lineStyle: filled ? LineStyle.Dotted : LineStyle.Dashed, axisLabelVisible: true, title: tpPrices.length > 1 ? \`TP\${order.sequence || index + 1}\${filled ? "✓" : ""}\` : filled ? "TP✓" : "TP" }); });`,
);
chart = chart.replace(
...
console.log("Custom chart toolbar removed; filled TP locks/markers and DCA edit cleanup prepared");
