...
replace(
  ...
  '  deviation:1, stepScale:1, volumeScale:1, takeProfit:1.5, takeProfitTargets:[{profitPct:1.5,allocationPct:100}], stopEnabled:false, stopPct:8, stopLossTimeoutSeconds:0, conditions:[],',
  'new form defaults',
);
replace(
...
replace(
  ...
  'setForm({name:bot.name,pair:bot.pair,pairs:bot.pairs?.length?bot.pairs:[bot.pair],allPairs:bot.allPairs,baseOrder:bot.baseOrder,safetyOrder:bot.safetyOrder,maxSafetyOrders:bot.maxSafetyOrders,limitSafetyOrders:bot.limitSafetyOrders,maxActiveTrades:bot.maxActiveTrades,deviation:bot.deviation,stepScale:bot.stepScale,volumeScale:bot.volumeScale,takeProfit:bot.takeProfit,takeProfitTargets:bot.takeProfitTargets?.length?bot.takeProfitTargets:[{profitPct:bot.takeProfit,allocationPct:100}],stopEnabled:bot.stopEnabled,stopPct:bot.stopPct,stopLossTimeoutSeconds:bot.stopLossTimeoutSeconds??0,conditions:bot.conditions??[]});',
  'bot form hydration',
);
replace(
...
replace(
  ...
  'takeProfit:form.takeProfitTargets[0]?.profitPct??form.takeProfit,takeProfitTargets:form.takeProfitTargets,stopEnabled:form.stopEnabled,stopPct:form.stopPct,stopLossTimeoutSeconds:form.stopLossTimeoutSeconds,conditions:form.conditions',
  'save payload',
);
replace(
...
const newExit=`<section className={cfg.card}><div className={cfg.cardHead}><div><h3>Exit settings</h3><p>Split the position across multiple Take Profit targets and optionally delay Stop Loss execution.</p></div><button type="button" className={cfg.addButton} disabled={form.takeProfitTargets.length>=8} onClick={addTp}>＋ Add TP</button></div>
      ...
      {accountKind==="real"&&<div className={cfg.liveNote}>Each TP executes as a real Binance Spot sell. Small partial exits still have to satisfy Binance minimum-order rules.</div>}
    </section>`;
...
console.log("Multi Take Profit and Stop Loss timeout configuration prepared");
