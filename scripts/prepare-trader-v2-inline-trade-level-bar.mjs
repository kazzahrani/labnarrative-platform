...
const replacementTop = `<div className={dca.tradeTop}><div className={dca.tradeIdentity}><strong style={{display:"flex",alignItems:"center",gap:8}}><CoinLogo symbol={trade.pair} size={22}/>{trade.pair}</strong><small>{trade.botName} · {trade.executionMode}</small></div><TradeLevelBar accountId={currentAccount.id} tradeId={trade.id} averagePrice={trade.averagePrice} livePrice={tradeState === "Active" ? trade.lastPrice : trade.exitPrice} stopLossPrice={trade.stopLossPrice} takeProfitPrice={trade.takeProfitPrice} active={tradeState === "Active"}/><div className={dca.tradeValue}><span>Invested</span><b>{money(trade.invested)}</b></div><div className={dca.tradeValue}><span>PnL</span><b className={trade.pnl >= 0 ? dca.green : dca.red}><TradePnlValue tradeId={trade.id} pnl={trade.pnl} active={tradeState === "Active"}/></b></div><TradeActionsV2 accountId={currentAccount.id} accountMode={currentAccount.mode} trade={trade} onChanged={() => loadWorkspace(true)}/></div>`;
...
console.log("Trader V2 compact trade rows with coin logos, exact levels, metadata and actions prepared");
