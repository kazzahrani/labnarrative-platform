...
home = replaceOnce(
  ...
  '{modules.slice(0, 6).map((module, index) => <button type="button" key={module.key} onClick={() => window.location.assign(selected ? prospectHref(module.href, selected) : module.href)} className={styles.flowItem} style={{textDecoration:"none",background:"transparent",border:0,padding:0,cursor:"pointer",color:"inherit",font:"inherit",pointerEvents:"auto",position:"relative",zIndex:2}}><span>{module.n}</span><b>{module.title}</b>{index < 5 ? <i>→</i> : null}</button>)}',
...
home = replaceOnce(
  ...
  '        <div className={styles.companyHead}><div><p>{selected.industry || "Systems prospect"}</p><h2>{selected.company_name}</h2><span>{[selected.city, selected.country].filter(Boolean).join(" · ")}</span></div><div className={styles.stageBadge}><span>Current stage</span><strong>{selectedStage.label}</strong><small>Updated {dateLabel(selected.updated_at)}</small></div></div>\n\n        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:14,position:"relative",zIndex:5,pointerEvents:"auto"}}>\n          <button type="button" onClick={() => window.location.assign(prospectHref("/admin/systems-outreach", selected))} style={{display:"inline-flex",alignItems:"center",borderRadius:10,padding:"10px 13px",background:"#aee94e",color:"#07161e",border:0,cursor:"pointer",fontSize:11,fontWeight:900,pointerEvents:"auto"}}>Open Acquisition →</button>\n          {selected.demo_status === "ready" ? <button type="button" onClick={() => window.open(`/systems/demos/${selected.slug}`, "_blank", "noopener,noreferrer")} style={{display:"inline-flex",alignItems:"center",borderRadius:10,padding:"10px 13px",background:"#35c7c1",color:"#07161e",border:0,cursor:"pointer",fontSize:11,fontWeight:900,pointerEvents:"auto"}}>Open Demo ↗</button> : <span style={{display:"inline-flex",alignItems:"center",borderRadius:10,padding:"10px 13px",border:"1px solid #294854",color:"#819aa3",fontSize:11,fontWeight:800}}>Demo not ready</span>}\n        </div>\n\n        <div className={styles.nextAction}>',
