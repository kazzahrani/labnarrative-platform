...
const creatorRewards = [
  ...
  ["Reddit", "2 high-quality original posts in relevant communities + each shared/reposted to up to 5 other relevant communities where permitted", "1 month Max"],
...
const steps = [
  ...
  ["04", "Convert customers", "Qualifying referred subscription payments earn 40% cash commission. Referral Max rewards continue to stack too."],
];
...
export default function AffiliatePage() {
  return <main className={styles.page}>
    ...
    <section className={affiliateStyles.rewardsSection} id="creator-rewards"><div className={styles.sectionIntro}><p className={styles.label}>Content rewards</p><h2>Publish useful LabNarrative content. Earn Max.</h2><p>Complete any approved package below to earn 1 month of Max. You can complete packages across multiple platforms.</p></div><div className={affiliateStyles.rewardTable}><div className={affiliateStyles.rewardHead}><span>Platform</span><span>Qualifying content</span><span>Reward</span></div>{creatorRewards.map(([platform, content, reward]) => <div className={affiliateStyles.rewardRow} key={platform}><strong>{platform}</strong><span>{content}</span><b>{reward}</b></div>)}</div></section>

    <section className={affiliateStyles.performanceSection}><div className={styles.sectionIntro}><p className={styles.label}>Performance bonuses</p><h2>When your content grows, your reward grows too.</h2><p>Performance rewards are additional to the content-package reward. Only the highest reached milestone applies; lower milestones do not stack.</p></div><div className={affiliateStyles.rewardTable}>{performanceRewards.map(([type, milestone, reward]) => <div className={affiliateStyles.performanceRow} key={type}><strong>{type}</strong><span>{milestone}</span><b>{reward}</b></div>)}</div></section>
