...
const EN: Record<string, string> = {
  ...
  "API قراءة فقط عندما يتوفر، أو ملف منظم عندما لا يتوفر.": "Use a read-only API when available, or a structured file when it is not.",
...
const INLINE_EN: Array<[string,string]> = [
  ["لوحة الثروة السعودية","Saudi Wealth Dashboard"],
  ["المحفظة الحقيقية","Real portfolio"],
  ["المحفظة التجريبية","Paper portfolio"],
  ["إدارة الثروة","Wealth management"],
...
function englishFormatting(value:string){
  return value
...
function translateDates(value:string){
  let next=englishFormatting(value);
  for(const [ar,en] of Object.entries(MONTHS)) next=next.replaceAll(ar,en);
  next=next.replace(/\sص$/," AM").replace(/\sم$/," PM");
  return next;
}

function dynamicEnglish(value:string){
  const normalized=englishFormatting(value);
  let m=value.match(/^(\d+|[٠-٩]+) أصل$/); if(m)return `${englishFormatting(m[1])} assets`;
  m=value.match(/^(\d+|[٠-٩]+) حساب · (\d+|[٠-٩]+) أصل$/); if(m)return `${englishFormatting(m[1])} accounts · ${englishFormatting(m[2])} assets`;
  m=value.match(/^عبر (\d+|[٠-٩]+) حسابات$/); if(m)return `Across ${englishFormatting(m[1])} accounts`;
  m=value.match(/^آخر تحديث (.+)$/); if(m)return `Last updated ${translateDates(m[1])}`;
  m=value.match(/^(\d+|[٠-٩]+) حدث خلال 5 سنوات$/); if(m)return `${englishFormatting(m[1])} events over 5 years`;
  m=value.match(/^تعديل: (.+)$/); if(m)return `Edit: ${m[1]}`;
  m=value.match(/^ربط (.+)$/); if(m)return `Connect ${m[1]}`;
  return normalized!==value?normalized:"";
}

function partialEnglish(value:string){
  let next=translateDates(value);
  for(const [ar,en] of INLINE_EN) next=next.replaceAll(ar,en);
  return englishFormatting(next);
}
...
function translateText(value:string){
  ...
  const exact=EN[core];
  if(exact)return `${leading}${englishFormatting(exact)}${trailing}`;
  const dynamic=dynamicEnglish(core);
  if(dynamic && !/[\u0600-\u06ff]/.test(dynamic))return `${leading}${dynamic}${trailing}`;
  return `${leading}${partialEnglish(core)}${trailing}`;
}
...
export default function WealthPreferences(){
  ...
  useEffect(()=>{
    setLanguage(localStorage.getItem(LANGUAGE_KEY)==="en"?"en":"ar");
  ...
  }:{language:"اللغة",theme:"الثيم",arabic:"العربية",english:"English",dark:"غامق",light:"فاتح"},[language]);

  return <div className="wealth-preferences" data-wealth-preferences="true" dir={language==="en"?"ltr":"rtl"}>
