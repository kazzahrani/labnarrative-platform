...
export default function BinanceConnectClient() {
  ...
  const sync = async () => {
    ...
    try {
      ...
      setMessage("تم تحديث أرصدة Binance والقيم الحالية وصافي الثروة بنجاح.");
      await load();
    } catch (reason) { setError(await edgeErrorMessage(reason)); }
    finally { setWorking(false); }
  };
  ...
  return <main className={styles.page} dir="rtl">
    <div className={styles.shell}>
      <header className={styles.header}>
        <div><Link href="/wealth/accounts" className={styles.back}>العودة إلى الحسابات</Link><p>ربط فعلي · قراءة فقط</p><h1>Binance</h1><span>مزامنة أرصدة Spot إلى ثروة دون تداول أو سحب أو تحويل أموال.</span></div>
      ...
      {loading ? <section className={styles.state}>جاري تحميل حالة الاتصال…</section> : <>
        ...
        {connection && <section className={styles.statusCard}>
          ...
          {connection.status === "error" && <p className={styles.note}>تم حفظ الاتصال والمفتاح المشفر. يمكنك إعادة المحاولة مباشرة بدون إدخال المفتاح من جديد.</p>}
          <div className={styles.statusActions}>
            <button type="button" onClick={sync} disabled={working || !retryable} className={styles.primary}>{working ? "جاري التحديث…" : connection.status === "error" ? "إعادة المزامنة" : "تحديث الآن"}</button>
        ...
        <section className={styles.grid}>
          <article className={styles.panel}>
            <small>إعداد المفتاح</small><h2>{connection ? "تغيير مفتاح Binance" : "ربط حساب Binance"}</h2>
          ...
          <article className={styles.panel}>
            <small>الصلاحيات المقبولة</small><h2>قراءة فقط، بلا استثناء.</h2>
        ...
        <section className={styles.whatSyncs}>
          ...
          <div className={styles.syncItems}><span>أرصدة Spot غير الصفرية</span><span>الكمية الحرة والمحجوزة</span><span>السعر الحالي</span><span>القيمة بالريال</span><span>Snapshot لصافي الثروة</span><span>سجل نجاح/فشل المزامنة</span></div>
