...
function isMarketEligible(r:Row){return(r.type==="سهم"||r.type==="ريت")&&r.symbol.trim().length>0}
function hasMarketPrice(r:Row){return isMarketEligible(r)&&r.unitPriceSar!==null&&r.unitPriceSar>0}
function rowValueOriginal(r:Row){const q=num(r.quantity);if(hasMarketPrice(r)&&q>0)return fromSar((r.unitPriceSar as number)*q,r.currency);if(isMarketEligible(r))return 0;return num(r.currentValue)}
...
export default function ManualAwaedClientV4(){
  ...
  const invalid=useMemo(()=>touched.filter(r=>{if(!r.name.trim())return true;const q=num(r.quantity),avg=num(r.averageCost);if(isMarketEligible(r))return !r.symbol.trim()||q<=0||avg<=0;return rowValueOriginal(r)<=0}),[touched]);
  ...
  async function save(){setError("");setSaved(false);setResult(null);if(invalid.length){setError("يوجد صف غير مكتمل. للأصول المتداولة أدخل الاسم والرمز والكمية ومتوسط التكلفة؛ السعر الحالي تجلبه ثروة تلقائيًا.");return}if(!touched.length&&Object.keys(baseline).length===0){setError("لا توجد بيانات للحفظ.");return}setSaving(true);try{const payload=touched.map(r=>{const q=num(r.quantity),avg=num(r.averageCost),mv=rowValueOriginal(r),managed=isMarketEligible(r);return{id:r.holdingId??null,name:r.name.trim(),symbol:r.symbol.trim()||null,asset_type:assetType(r.type,r.currency),original_currency:r.currency,quantity:q>0?q:null,average_cost:avg>0?avg:null,market_value:managed?null:mv>0?mv:null,keep_market_price:managed}});const{data,error:rpcError}=await browserSupabase.rpc("wealth_reconcile_awaed_v4",{p_holdings:payload,p_base_currency:baseCurrency});if(rpcError)throw rpcError;const r=data as{added?:number;updated?:number;removed?:number;adjustments?:number};setResult({added:Number(r?.added??0),updated:Number(r?.updated??0),removed:Number(r?.removed??0),adjustments:Number(r?.adjustments??0)});setSaved(true);await load()}catch(reason){setError(reason instanceof Error?reason.message:"تعذر مطابقة محفظة عوائد.")}finally{setSaving(false)}}
  ...
  return<section className={styles.card}>
    ...
    <div className={styles.heading}><span>السوق السعودي + السوق الأمريكي</span><h2>أدخل الأصل بعملته الأصلية، واترك السعر لثروة.</h2><p>للأسهم والـREIT يكفي الرمز والكمية ومتوسط التكلفة؛ السعر والقيمة الحالية تأتي تلقائيًا من Market Data Engine. استخدم القيمة الحالية يدويًا فقط للأصول غير المسعّرة في السوق.</p></div>
    <div className={styles.tableWrap}><div className={styles.headerRow}><span>الاستثمار</span><span>النوع</span><span>الرمز</span><span>العملة</span><span>الكمية</span><span>متوسط التكلفة</span><span>القيمة الحالية</span><span></span></div><div className={styles.rows}>{rows.map(row=>{const managed=isMarketEligible(row);const priced=hasMarketPrice(row);const display=rowValueOriginal(row);const mark=row.currency==="USD"?"$":"ر.س";return<div className={styles.row} key={row.key}>
      ...
      <label><span className={styles.mobileLabel}>النوع</span><select value={row.type} onChange={e=>patch(row.key,{type:e.target.value as AssetKind,unitPriceSar:null})}>{kinds.map(k=><option key={k}>{k}</option>)}</select></label>
      <label><span className={styles.mobileLabel}>الرمز</span><input value={row.symbol} onChange={e=>patch(row.key,{symbol:e.target.value,unitPriceSar:null})} placeholder={row.currency==="USD"?"AAPL":"2222"} dir="ltr"/></label>
      <label><span className={styles.mobileLabel}>العملة</span><select value={row.currency} onChange={e=>patch(row.key,{currency:e.target.value as WealthCurrency,unitPriceSar:null})}><option value="SAR">ر.س</option><option value="USD">$</option></select></label>
      ...
      <label><span className={styles.mobileLabel}>القيمة الحالية</span>{managed?<div className={styles.autoValue}>{priced?<strong>{formatWealthMoney(toSar(display,row.currency),row.currency)}</strong>:<strong>تلقائي</strong>}<small>{priced?"سعر السوق الحالي":"سيُجلب بعد الحفظ"}</small></div>:<div className={styles.moneyInput}><input value={row.currentValue} onChange={e=>numberPatch(row.key,"currentValue",e.target.value)} inputMode="decimal" dir="ltr" placeholder="0"/><small>{mark}</small></div>}</label>
    ...
    <div className={styles.footer}><p>ثروة تحفظ التكلفة والعملة الأصلية، بينما السعر السوقي للأصول المتداولة يأتي من Market Data Engine ويُحوّل داخليًا إلى الريال للحساب الموحد.</p><button className={styles.primary} type="button" onClick={save} disabled={saving||invalid.length>0}>{saving?"جارٍ المطابقة…":"حفظ ومطابقة"}</button></div>
  </section>;
}