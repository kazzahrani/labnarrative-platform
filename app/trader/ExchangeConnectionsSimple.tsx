...
export default function ExchangeConnectionsSimple({realAccount,onConnectBinance,onBackOverview}:Props){
  ...
  const load=useCallback(async()=>{
    ...
    try{const result=await invokeMulti("status_all");const next={bybit:null,okx:null,kraken:null,kucoin:null,coinbase:null} as Record<GenericProvider,Connection>;for(const row of result.connections??[])next[row.provider]=row.connection;setConnections(next)}catch(caught){setError(friendly(caught))}finally{setLoading(false)}
  },[realAccount]);
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.note}>Connected exchange balances and venue identity are shared with Portfolio, Automations and Positions. Live order routing is enabled only after the protected execution path for that venue is active.</div>
    ...
    {selected&&<div className={styles.backdrop} onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><section className={styles.modal}>
      ...
      <form className={styles.form} onSubmit={connect}>
        ...
        <p>Enable Spot trading and reading only. Keep withdrawals, transfers, futures, margin and other unrelated permissions disabled.</p>
