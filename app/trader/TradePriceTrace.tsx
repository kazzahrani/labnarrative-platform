...
const GREEN = "#6CB38C";
const RED = "#B26F74";
const GUIDE = "rgba(188,188,188,.24)";
...
async function candlesFor(pair:string,openedAt:string,closedAt:string|null){
  ...
  const url=`https://data-api.binance.vision/api/v3/klines?symbol=${encodeURIComponent(symbol)}&interval=${interval}&startTime=${Math.max(0,Math.floor(start))}&endTime=${Math.floor(end)}&limit=500`;
  const response=await fetch(url,{cache:"no-store"});
  if(!response.ok)throw new Error(`binance_chart_${response.status}`);
  const raw=await response.json() as unknown[];
  return raw.flatMap((row)=>{
    if(!Array.isArray(row))return [];
    const time=Number(row[0]),close=Number(row[4]);
...
export default function TradePriceTrace({accountId,tradeId,pair,averagePrice,livePrice,stopLossPrice,takeProfitPrice,openedAt,closedAt,active}:Props){
  ...
  const model=useMemo(()=>{
    ...
    [...dcaBySequence.entries()].sort((a,b)=>a[0]-b[0]).slice(0,6).forEach(([sequence,price])=>levels.push({key:`dca-${sequence}`,label:`D${sequence}`,price,kind:"dca"}));
  ...
  const width=520,height=118,labelWidth=46,padX=10,padY=8,plotW=width-labelWidth-padX*2,plotH=height-padY*2;
  ...
  return <div className={styles.wrap} aria-label="Position price movement">
    <svg className={styles.chart} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img">
      ...
      {model.levels.map((level)=><g key={level.key}>
        <line x1={padX} x2={levelEnd} y1={y(level.price)} y2={y(level.price)} stroke={GUIDE} strokeWidth="1" strokeDasharray="4 5"/>
        <text x={levelEnd+8} y={y(level.price)+3} className={styles.levelLabel}>{level.label}</text>
      </g>)}
      {model.series.slice(1).map((point,index)=>{
        ...
        return <line key={`${point.time}-${index}`} x1={x(index)} y1={y(previous.close)} x2={x(index+1)} y2={y(point.close)} stroke={positive?GREEN:RED} strokeWidth="1.8" strokeLinecap="round"/>;
      })}
