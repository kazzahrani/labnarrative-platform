...
export default function TraderV2FullShell() {
  ...
  const renderBotEditor = () => <form className={dca.detailBody} onSubmit={saveBot}>
    ...
    <div className={dca.editorGrid}><label><span>Bot name</span><input value={botForm.name} onChange={(event) => setBotForm((value) => ({...value,name:event.target.value}))}/></label><label><span>Pair</span><select value={botForm.pair} onChange={(event) => setBotForm((value) => ({...value,pair:event.target.value}))}>{pairOptions.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Base order</span><div className={dca.inputUnit}><input type="number" min="1" step="0.01" value={botForm.baseOrder} onChange={(event) => setBotForm((value) => ({...value,baseOrder:Number(event.target.value)}))}/><b>USDT</b></div></label><label><span>Start condition</span><select value="Immediately" disabled><option>Immediately</option></select></label></div>
