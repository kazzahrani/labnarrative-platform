...
const GREEN="#55d99a", RED="#df7b84", BLUE="#79a2ef", GOLD="#d6a956", VIOLET="#a98be8";
...
function buildCapitalTimeline(trades:Trade[]){
  ...
  trades.forEach(t=>{const open=t.openedAt?Date.parse(t.openedAt):NaN;const close=t.closedAt?Date.parse(t.closedAt):NaN;const c=Math.max(0,finite(t.capital));if(!c||!Number.isFinite(open)||!Number.isFinite(close))return;events.push({at:open,delta:c},{at:close,delta:-c});});
  events.sort((a,b)=>a.at-b.at||b.delta-a.delta);let current=0;return events.map(e=>{current=Math.max(0,current+e.delta);return{at:e.at,value:current};});
}
function rolling(trades:Trade[],windowSize=20){
  ...
  return sorted.map((_,i)=>{const start=Math.max(0,i-windowSize+1),rows=sorted.slice(start,i+1);const capital=rows.reduce((s,t)=>s+Math.max(0,finite(t.capital)),0);const pnl=rows.reduce((s,t)=>s+finite(t.pnl),0);const wins=rows.filter(t=>finite(t.pnl)>0).length;const gp=rows.reduce((s,t)=>s+Math.max(0,finite(t.pnl)),0),gl=Math.abs(rows.reduce((s,t)=>s+Math.min(0,finite(t.pnl)),0));return{roi:capital>0?pnl/capital*100:0,win:rows.length?wins/rows.length*100:0,pf:gl>0?gp/gl:(gp>0?4:0)};});
}
function histogram(trades:Trade[],bins=12){
  ...
  let min=Math.min(...values),max=Math.max(...values);if(min===max){min-=.5;max+=.5;}const width=(max-min)/bins;const out=Array.from({length:bins},(_,i)=>({from:min+i*width,to:min+(i+1)*width,count:0}));values.forEach(v=>{const i=Math.min(bins-1,Math.max(0,Math.floor((v-min)/width)));out[i].count++;});return out;
}
function heat(trades:Trade[]){
  const cells=Array.from({length:168},()=>({count:0,pnl:0,roi:0,capital:0}));
  trades.forEach(t=>{if(!t.closedAt)return;const d=new Date(t.closedAt);const i=d.getDay()*24+d.getHours();cells[i].count++;cells[i].pnl+=finite(t.pnl);cells[i].capital+=Math.max(0,finite(t.capital));});
  cells.forEach(c=>{c.roi=c.capital>0?c.pnl/c.capital*100:0;});return cells;
}
...
export default function AdvancedBotAnalytics({range,automation,automations,detail}:Props){
  ...
  const timeline=buildCapitalTimeline(trades),capitalValues=timeline.map(p=>p.value),peakCapital=Math.max(0,...capitalValues);
  const maxCapital=automation.maxCapitalMode==="fixed"?finite(automation.maxCapital,0):0;
  ...
  const excursions=trades.filter(t=>t.mfePct!=null&&Number.isFinite(t.mfePct)&&t.maePct!=null&&Number.isFinite(t.maePct));
  const maxMfe=Math.max(1,...excursions.map(t=>finite(t.mfePct))),maxMae=Math.max(1,...excursions.map(t=>Math.abs(finite(t.maePct))));

  return <section className={styles.suite}>
    ...
    <div className={styles.grid}>
      <article className={styles.card}>
        ...
        {capitalValues.length?<svg key={`${range}-capital-${trades.length}`} className={styles.chart} viewBox="0 0 600 150" preserveAspectRatio="none"><path d={areaPath(scalePoints(capitalValues),138)} className={styles.areaBlue}/><path d={path(scalePoints(capitalValues))} className={styles.drawBlue}/>{maxCapital>0&&<line x1="10" x2="590" y1={Math.max(10,138-Math.min(128,maxCapital/Math.max(maxCapital,peakCapital)*128))} y2={Math.max(10,138-Math.min(128,maxCapital/Math.max(maxCapital,peakCapital)*128))} className={styles.limitLine}/>}</svg>:<div className={styles.empty}>No closed-trade capital timeline in this range.</div>}
      ...
      <article className={styles.card}>
        ...
        <svg key={`${range}-risk-${risk.length}`} className={styles.chart} viewBox="0 0 600 150" preserveAspectRatio="none">
          <line x1="42" x2="590" y1="128" y2="128" className={styles.axis}/><line x1="42" x2="42" y1="10" y2="128" className={styles.axis}/>
          {risk.map(({a,x,y,size})=>{const cx=42+x/maxRisk*535,cy=128-(y-minRoi)/(maxRoi-minRoi||1)*112,r=5+Math.sqrt(size/maxBubble)*9;return <circle key={a.id} cx={cx} cy={cy} r={r} className={a.id===automation.id?styles.focusBubble:styles.bubble}><title>{a.name} · ROI {pct(y)} · Max DD {pct(-x)} · capital {money(size)}</title></circle>})}
        </svg>
