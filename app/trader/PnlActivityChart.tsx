...
export default function PnlActivityChart({accountId,range,scope="all",type="all",botId="",embedded=false}:Props){
  ...
  const [resolution,setResolution]=useState<Resolution>(()=>defaultResolution(range));const [measure,setMeasure]=useState<Measure>("pnl");const [hover,setHover]=useState<number|null>(null);const svgRef=useRef<SVGSVGElement|null>(null);
  ...
  const body=<>
    <div className={styles.controls}><div className={styles.segment}>{(["pnl","roi"] as Measure[]).map(m=><button key={m} className={measure===m?styles.active:""} onClick={()=>setMeasure(m)}>{m==="pnl"?"PnL":"ROI"}</button>)}</div><div className={styles.segment}>{(["daily","weekly","monthly"] as Resolution[]).map(r=><button key={r} className={resolution===r?styles.active:""} onClick={()=>setResolution(r)}>{r[0].toUpperCase()+r.slice(1)}</button>)}</div></div>
    ...
    <div className={styles.chartWrap}>
      {loading?<div className={styles.empty}>Loading performance rhythm…</div>:error?<div className={styles.error}>{error}</div>:!rows.length?<div className={styles.empty}>No closed trades in this period.</div>:<>
        <svg ref={svgRef} className={styles.chart} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" onMouseMove={onMove} onMouseLeave={()=>setHover(null)}>
          ...
          {ticks(0,maxTrades,5).map(v=><text key={`r-${v}`} x={W-R+10} y={countY(v)+4} className={styles.tick}>{Math.round(v)}</text>)}
          ...
          <path d={countPath} className={styles.activityLine}/>{rows.map((r,i)=><circle key={`c-${r.at}`} cx={x(i)} cy={countY(r.trades)} r="2.6" className={styles.activityDot}/>)}
          ...
          <text x={(L+W-R)/2} y={H-2} textAnchor="middle" className={styles.axisTitle}>Time</text><text x="13" y={(T+H-B)/2} textAnchor="middle" transform={`rotate(-90 13 ${(T+H-B)/2})`} className={styles.axisTitle}>{measure==="pnl"?"Realized PnL (USD)":"Realized ROI (%)"}</text><text x={W-10} y={(T+H-B)/2} textAnchor="middle" transform={`rotate(90 ${W-10} ${(T+H-B)/2})`} className={styles.axisTitle}>Closed trades</text>
    ...
    </div><div className={styles.legend}><span><i className={styles.barKey}/>Period {measure==="pnl"?"PnL":"ROI"}</span><span><i className={styles.lineKey}/>Closed trades</span></div>
  </>;
