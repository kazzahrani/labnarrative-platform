...
function getSrcFromTag(tag: string) {
  ...
  const candidates = [srcset ? fromSrcset(srcset) : "",getAttr(tag,"data-src"),getAttr(tag,"data-lazy-src"),getAttr(tag,"src")].filter(Boolean).filter((value) => !/1x1-gray\.gif/i.test(value));
  return candidates[0] || "";
}
...
function resolveMarkupImage(markup: string, sourceUrl: URL) {
  ...
  for (const tag of [...sourceTags, ...imgTags]) {
...
function findPortrait(html: string, sourceUrl: URL, personName: string) {
  ...
  const rankedPictures = pictureBlocks.map((block) => ({block,score:scoreImage(block.match(/<img\b[^>]*>/i)?.[0] || "",personName)})).filter((item) => item.score > 0).sort((a,b)=>b.score-a.score);
  for (const item of rankedPictures) { const resolved=resolveMarkupImage(item.block,sourceUrl); if(resolved) return resolved; }
  const tags = html.match(/<img\b[^>]*>/gi) ?? [];
  const ranked = tags.map((tag)=>({tag,score:scoreImage(tag,personName)})).filter((item)=>item.score>0).sort((a,b)=>b.score-a.score);
  for (const item of ranked) { const raw=getSrcFromTag(item.tag); if(!raw||raw.startsWith("data:")) continue; try{return new URL(raw,sourceUrl).toString();}catch{continue;} }
  return "";
}

function validatedHttpsUrl(value: string) {
  try { const url=new URL(value); if(url.protocol!=="https:"||!sourceHostAllowed(url.hostname)) return null; return url; } catch { return null; }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    if(directImage){const directImageUrl=validatedHttpsUrl(directImage);if(!directImageUrl)return NextResponse.json({error:"image_not_allowed"},{status:403});return await proxyImage(directImageUrl);}
