...
async function callFunction(name: string, authorization: string, apikey: string, supabaseUrl: string, body: unknown, timeoutMs = 12_000) {
  return await fetch(`${supabaseUrl.replace(/\/$/, "")}/functions/v1/${encodeURIComponent(name)}`, {
    method: "POST",
    headers: { authorization, apikey, "content-type": "application/json", "x-client-info": "labnarrative-platform-v1-position-edit/2" },
...
export async function POST(request: NextRequest) {
  ...
  try {
    const positionsResponse = await callFunction("trader-v2-positions-read", authorization, apikey, supabaseUrl, {});
    ...
    const match = (positionsPayload.positions as CorePosition[]).find((position) => String(position.client_id || "") === tradeId || String(position.trade_id || "") === tradeId);
    const positionId = String(match?.trade_id || "");
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(positionId)) return json({ error: "core_v2_position_id_unavailable" }, 404);

    const submitResponse = await callFunction("trader-v2-position-edit-submit", authorization, apikey, supabaseUrl, {
      positionId,
    ...
    const submitPayload = object(await parseResponse(submitResponse));
    if (!submitResponse.ok || submitPayload.ok !== true) return json({ error: String(submitPayload.error || `position_edit_http_${submitResponse.status}`) }, submitResponse.ok ? 400 : submitResponse.status);
    const commandId = String(object(submitPayload.command).id || "");
    if (!commandId) return json({ error: "position_edit_command_missing" }, 500);

    for (let attempt = 0; attempt < 20; attempt += 1) {
      if (attempt > 0) await wait(750);
      const statusResponse = await callFunction("trader-v2-position-edit-status", authorization, apikey, supabaseUrl, { commandId }, 8_000);
      const statusPayload = object(await parseResponse(statusResponse));
      if (!statusResponse.ok || statusPayload.ok !== true) continue;
      const command = object(statusPayload.command);
      const status = String(command.status || "");
      if (status === "succeeded") return json({ ok: true, pending: false, command, result: command.result }, 200);
      if (status === "failed") return json({ error: String(command.error_code || "position_edit_failed"), command }, 409);
    }

    return json({ ok: true, pending: true, command: { id: commandId, status: "queued" } }, 202);
  } catch (error) {
    const name = error instanceof Error ? error.name : "position_edit_transport_failed";
    if (name === "TimeoutError" || name === "AbortError") return json({ error: "position_edit_timeout" }, 504);
