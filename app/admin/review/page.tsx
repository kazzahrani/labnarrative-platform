...
type Dashboard = { counts:{ finalReview:number; published:number; blocked:number }; runs:Run[] };
type PublishResult = { ok:boolean; runId:string; publicUrl:string; outreachSent:boolean; outreachDraft?:{ status?:string; recipientEmail?:string } };
...
export default function EngineV3ReviewPage(){
  ...
  const load=useCallback(async(activeSession:Session)=>{try{setDashboard(await rpc<Dashboard>(activeSession,"engine_v3_admin_dashboard"));setNotice("")}catch(error){setNotice(error instanceof Error?error.message:"Final Review could not be loaded.")}},[]);
  ...
  async function approve(run:Run){
    ...
    try{const result=await rpc<PublishResult>(session,"engine_v3_admin_approve_publish",{p_run_id:run.runId,p_note:null});if(!result.ok||result.outreachSent)throw new Error("Publication did not return the expected safe draft state.");window.location.href=`/admin/outreach/${run.runId}`}
  ...
  async function block(run:Run){
    ...
    try{await rpc(session,"engine_v3_admin_block",{p_run_id:run.runId,p_reason:reason});setNotice(`${run.piName} was blocked: ${reason}`);await load(session)}catch(error){setNotice(error instanceof Error?error.message:"Block failed.")}finally{setBusy("")}
  }
  ...
  async function revise(run:Run){
    ...
    try{await rpc(session,"engine_v3_admin_return_to_production",{p_run_id:run.runId,p_note:note||null});setNotice(`${run.piName} was returned to production.`);await load(session)}catch(error){setNotice(error instanceof Error?error.message:"Return to production failed.")}finally{setBusy("")}
  }
  ...
  return <main className={styles.page}>
    <header className={styles.topbar}><div><Link className={styles.brand} href="/admin">LabNarrative</Link><span>Final Review</span></div><nav><Link href="/admin/automation">Production</Link><Link href="/admin/sites">Websites</Link><Link href="/admin/discovery">Discovery</Link></nav></header>
    <section className={styles.content}>
      <div className={styles.hero}><div><p className={styles.kicker}>Human publication gate</p><h1>Review. Publish. Prepare outreach.</h1><p>Nothing is published until you approve it. Approval creates an editable outreach draft, but never sends it.</p></div><button onClick={()=>void load(session)} type="button">Refresh</button></div>
      {notice?<p className={styles.notice}>{notice}</p>:null}
      <section className={styles.stats}><article><span>Awaiting review</span><strong>{finalReview.length}</strong></article><article><span>Published by v3</span><strong>{dashboard?.counts.published??published.length}</strong></article><article><span>Blocked</span><strong>{dashboard?.counts.blocked??0}</strong></article></section>
