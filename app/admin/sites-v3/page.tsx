...
function dataIssues(site: SiteRow, run?: V3Run): string[] {
  ...
  if (run) {
    for (const key of REQUIRED_KEYS) {
      ...
      if (value == null || value === "" || (ARRAY_KEYS.includes(key) && !Array.isArray(value)) || (["pages","theme","design"].includes(key) && (typeof value !== "object" || Array.isArray(value)))) {
...
export default function SiteMonitorV3Page() {
  ...
  const enriched = useMemo(() => sites.map((site) => {
    const run = runBySite.get(site.id);
    const prospect = prospectBySite.get(site.id);
    const issues = dataIssues(site, run);
  ...
  const visible = useMemo(() => {
    ...
    const result = enriched.filter((row) => {
      const { site, run, prospect, issues } = row;
    ...
    return result.sort((a,b) => {
      if (sort === "name") return piName(a.site,a.prospect,a.run).localeCompare(piName(b.site,b.prospect,b.run));
      if (sort === "problems") return b.issues.length - a.issues.length || Date.parse(b.site.updated_at)-Date.parse(a.site.updated_at);
      return Date.parse(b.site.updated_at)-Date.parse(a.site.updated_at);
    });
  ...
  async function runHealthChecks() {
    ...
    for (let i = 0; i < targets.length; i += 25) {
      ...
      for (const item of payload.results || []) next[item.id] = item;
  ...
  return (
    <main className={styles.page}>
      <header className={styles.topbar}>
        <div className={styles.topbarLeft}><Link className={styles.brand} href="/admin">LabNarrative</Link><span>Website Monitor v3</span></div>
        <nav className={styles.nav}><Link href="/admin/review">Final Review</Link><Link href="/admin/automation">Production</Link><Link href="/admin/sales">Sales</Link></nav>
      </header>

      <section className={styles.content}>
        <div className={styles.hero}>
          <div><p className={styles.kicker}>Site operations</p><h1>Every website. One operational truth.</h1><p>Engine v3 workflow state, publication state, renderer-contract integrity, domain health and portrait rendering are monitored separately so real problems surface immediately.</p></div>
          <div className={styles.heroActions}><button className={styles.buttonSecondary} onClick={() => void load()} disabled={loading}>{loading ? "Refreshing…" : "Refresh data"}</button><button className={styles.button} onClick={() => void runHealthChecks()} disabled={checking}>{checking ? "Checking sites…" : "Run health checks"}</button></div>
        </div>

        <section className={styles.metrics}>
          {[{k:"all" as Filter,l:"Active sites",v:metrics.total,s:"Non-archived"},{k:"live" as Filter,l:"Public",v:metrics.live,s:"Published concepts"},{k:"private" as Filter,l:"Private",v:metrics.private,s:"Draft concepts"},{k:"v3" as Filter,l:"Engine v3",v:enriched.filter(x=>x.run).length,s:`${metrics.finalReview} in Final Review`},{k:"problems" as Filter,l:"Problems",v:metrics.problems,s:"Needs attention"}].map((m)=><button key={m.k} className={`${styles.metric} ${filter===m.k?styles.metricActive:""}`} onClick={()=>setFilter(m.k)}><span>{m.l}</span><strong>{m.v}</strong><small>{m.s}</small></button>)}
        </section>
          ...
          <table className={styles.table}>
            ...
            <tbody>{visible.map(({site,run,prospect,issues})=>{
              ...
              return <tr id={`site-${site.id}`} key={site.id}>
                ...
                <td><div className={styles.actions}>{pub?<a href={publicUrl(site,run)} target="_blank" rel="noreferrer">Open site</a>:null}<Link href={`/admin/preview/${site.slug}`}>Preview</Link>{run?.state==="final_review"?<Link href="/admin/review">Final Review</Link>:null}{run&&["published","completed"].includes(run.state)?<Link href={`/admin/outreach/${run.runId}`}>Outreach</Link>:null}<Link href="/admin/sales">Sales</Link></div></td>
              </tr>;
            })}</tbody>
          </table>
          {!visible.length?<div className={styles.empty}>No websites match this view.</div>:null}
        </div>
        <p className={styles.footerNote}>Health checks are live HTTP checks against LabNarrative public domains plus browser image-load checks for PI portraits. Renderer-contract checks are applied to Engine v3 sites only; legacy sites remain visible without being judged against the v3 contract.</p>
      </section>
    </main>
  );
}
