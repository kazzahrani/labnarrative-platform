...
async function sendThroughLabNarrative(session: Session, productionRunId: string): Promise<SendResult> {
  const response = await fetch(`${supabaseUrl}/functions/v1/operator-send-outreach`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      apikey: supabaseKey,
    ...
    body: JSON.stringify({ runId: productionRunId }),
    cache: "no-store",
  });
  const payload = await response.json().catch(() => ({})) as SendResult;
  if (!response.ok || payload.ok !== true) {
    throw new Error(payload.error || `Email delivery failed (${response.status}).`);
  }
  return payload;
}
...
export default function EngineV3OutreachDraftPage() {
  ...
  const [action, setAction] = useState<"" | "save" | "send" | "private">("");
  ...
  async function persistDraft(activeSession: Session, showNotice = true): Promise<OutreachDraft> {
    const row = await rpc<OutreachDraft>(activeSession, "engine_v3_admin_outreach_save", {
      p_run_id: runId,
    ...
    if (showNotice) setNotice("Draft saved. No email was sent.");
    return row;
  }

  async function save(event: FormEvent) {
    event.preventDefault();
    if (!session || action || draft?.status !== "draft") return;
    setAction("save");
    try {
      await persistDraft(session, true);
    } catch (error) {
  ...
  async function sendNow() {
    ...
    if (!email) {
      setNotice("Add the verified recipient email before sending.");
      return;
    }
    ...
    try {
      ...
      const confirmation = window.prompt(
        `This will send the email NOW through LabNarrative.\n\nRecipient: ${saved.recipientEmail}\nSubject: ${saved.subject}\n\nTo confirm, type the exact recipient email:`,
      );
      if (confirmation === null) {
        setNotice("Send cancelled. The latest edits were saved as a draft.");
        return;
      }
      if (confirmation.trim().toLowerCase() !== saved.recipientEmail.trim().toLowerCase()) {
        throw new Error("The confirmation email did not match the exact recipient. Nothing was sent.");
      }

      await rpc<boolean>(session, "authorize_operator_send", {
        p_run_id: saved.productionRunId,
        p_recipient_email: saved.recipientEmail,
      });
      const result = await sendThroughLabNarrative(session, saved.productionRunId);
      setNotice(result.alreadySent
        ? "This outreach email had already been sent."
        : `Email sent successfully to ${result.recipient || saved.recipientEmail}.`);
      await load(session);
    } catch (error) {
  ...
  async function confirmPrivateSend() {
    ...
    const email = recipientEmail.trim().toLowerCase();
    if (!email) {
      setNotice("Add the recipient email before recording a private send.");
      return;
    }
    ...
    try {
      ...
      const confirmed = window.confirm(
        `Confirm that you ALREADY sent this outreach yourself from a personal email account.\n\nRecipient: ${saved.recipientEmail}\n\nLabNarrative will not send an email. This only records the outreach as sent.`,
      );
      if (!confirmed) {
        setNotice("Private-send confirmation cancelled. The latest edits were saved as a draft.");
        return;
      }
      ...
      setNotice(error instanceof Error ? error.message : "The private send could not be recorded.");
    } finally {
      setAction("");
    }
  }
  ...
  return (
    <main className={styles.page}>
      <header className={styles.topbar}>
        <div><Link className={styles.brand} href="/admin">LabNarrative</Link><span>Outreach</span></div>
      ...
      <section className={styles.content}>
        <div className={styles.hero}>
          <div><p className={styles.kicker}>Human-controlled outreach</p><h1>Review, then choose how to send.</h1><p>Saving never sends. “Send Email Now” uses the protected LabNarrative delivery flow only after an exact-recipient confirmation. “Confirm Sent From Personal Email” only records an email you already sent yourself.</p></div>
          {draft?.publicUrl ? <a href={draft.publicUrl} target="_blank" rel="noreferrer">Open published concept ↗</a> : null}
        </div>

        <div className={styles.safety} data-status={draft?.status || "draft"}>
          <strong>{draft?.status === "sent" ? "Outreach complete:" : "Human gate:"}</strong>
          <span>{draft?.status === "sent"
            ? "This outreach is already recorded as sent. The editor is now read-only."
            : `Outreach status is ${draft?.status || "draft"}. Nothing is sent unless you explicitly choose a send action below.`}</span>
        </div>
        ...
        {draft ? (
          <form className={styles.form} onSubmit={save}>
            ...
            <div className={styles.actions}>
              {editable ? <>
                <button className={styles.saveButton} disabled={Boolean(action)} type="submit">{action === "save" ? "Saving…" : "Save Draft"}</button>
                <button className={styles.sendButton} disabled={Boolean(action) || !recipientEmail.trim()} onClick={() => void sendNow()} type="button">{action === "send" ? "Sending…" : "Send Email Now"}</button>
                <button className={styles.privateButton} disabled={Boolean(action) || !recipientEmail.trim()} onClick={() => void confirmPrivateSend()} type="button">{action === "private" ? "Recording…" : "Confirm Sent From Personal Email"}</button>
              </> : null}
