...
begin
  ...
  for v_r in
    ...
    where r.state='producing'
      and coalesce(r.checkpoint_at,r.updated_at,r.started_at) < now() - make_interval(mins=>coalesce(v_stale_minutes,45))
    order by p.queued_at asc nulls last,
             p.created_at asc,
             coalesce((select min(e.id) from labnarrative_engine_v3.events e where e.run_id=r.id and e.event_type='claimed'),9223372036854775807::bigint) asc,
             r.started_at asc,
             r.id asc
