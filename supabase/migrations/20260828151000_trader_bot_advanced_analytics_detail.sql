...
begin
  ...
  with closed as (
    select id, pair, coalesce(realized_pnl,0)::numeric as pnl,
      ...
      trailing_peak_price::numeric as trailing_peak_price,
      case when jsonb_typeof(client_state->'mfePct')='number' then (client_state->>'mfePct')::numeric else null end as mfe_pct,
      case when jsonb_typeof(client_state->'maePct')='number' then (client_state->>'maePct')::numeric else null end as mae_pct,
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'id',id,'pair',pair,'pnl',pnl,'roi',roi,'capital',capital,'durationMinutes',duration_minutes,
      'openedAt',opened_at,'closedAt',closed_at,'averagePrice',average_price,'trailingPeakPrice',trailing_peak_price,
      'mfePct',mfe_pct,'maePct',mae_pct
    ) order by closed_at asc),'[]'::jsonb) as value
    from (select * from closed order by closed_at desc limit 1000) x
  ), depth_rows as (
  ...
  select jsonb_build_object(
    'recentTrades',recent.value,
    'analyticsTrades',analytics_json.value,
    'dcaDepth',depth_json.value,
    'monthly',month_json.value,
    'weekdays',weekday_json.value,
    'capital',capital_json.value,
    'signals',signal_json.value
  ) into v_result
  from recent cross join analytics_json cross join depth_json cross join month_json cross join weekday_json cross join capital_json cross join signal_json;
