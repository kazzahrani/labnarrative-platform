create table if not exists public.sales_lead_workspaces (
  id uuid primary key default gen_random_uuid(),
  prospect_id uuid not null unique references public.prospects(id) on delete cascade,
  stage text not null default 'replied' check (stage in ('replied','interested','meeting_scheduled','proposal_sent','client','not_pursuing')),
  ...
  proposal_status text not null default 'not_started' check (proposal_status in ('not_started','drafting','ready','sent','accepted','declined')),
  proposal_sent_at timestamptz,
  proposal_amount numeric(12,2),
  proposal_currency text not null default 'USD',
  payment_status text not null default 'not_requested' check (payment_status in ('not_requested','deposit_requested','deposit_received','paid_in_full','refunded')),
...
create index if not exists sales_lead_workspaces_stage_idx on public.sales_lead_workspaces(stage, next_action_due_at);
alter table public.sales_lead_workspaces enable row level security;
revoke all on public.sales_lead_workspaces from public, anon, authenticated;
...
begin
  ...
  v_stage:=case when v_site.outreach_status in ('replied','interested','meeting_scheduled','proposal_sent','client','not_pursuing') then v_site.outreach_status when v_prospect.status='interested' then 'interested' else 'replied' end;
...
begin
  if not public.is_labnarrative_admin() then raise exception 'Administrator access required'; end if;
  if p_stage not in ('replied','interested','meeting_scheduled','proposal_sent','client','not_pursuing') then raise exception 'Invalid sales stage'; end if;
  ...
  if v_site_id is not null then update public.sites set outreach_status=p_stage,updated_at=now() where id=v_site_id; end if;
  update public.prospects set status=case when p_stage='not_pursuing' then 'rejected' when p_stage='replied' then 'replied' else 'interested' end,updated_at=now() where id=p_prospect_id;
...
revoke all on function public.sales_lead_workspace_get(uuid) from public,anon;
revoke all on function public.sales_lead_workspace_save(uuid,text,text,text,timestamptz,timestamptz,text,text,text,text,timestamptz,numeric,text,text,numeric,numeric,timestamptz) from public,anon;
grant execute on function public.sales_lead_workspace_get(uuid) to authenticated;
grant execute on function public.sales_lead_workspace_save(uuid,text,text,text,timestamptz,timestamptz,text,text,text,text,timestamptz,numeric,text,text,numeric,numeric,timestamptz) to authenticated;
