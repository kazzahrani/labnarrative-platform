...
begin
  ...
  if coalesce(v_site.content->>'title','')='' then raise exception 'labsite_title_missing'; end if;
  if coalesce(v_site.content->>'eyebrow','')='' then raise exception 'labsite_eyebrow_missing'; end if;
  if coalesce(v_site.content->>'institution','')='' then raise exception 'labsite_institution_missing'; end if;
  if jsonb_typeof(v_site.content->'focusAreas')<>'array' or jsonb_array_length(v_site.content->'focusAreas')<1 then raise exception 'labsite_focus_areas_missing'; end if;
  if jsonb_typeof(v_site.content->'team')<>'array' or jsonb_array_length(v_site.content->'team')<1 then raise exception 'labsite_team_missing'; end if;
  if jsonb_typeof(v_site.content->'members')<>'array' or jsonb_array_length(v_site.content->'members')<1 then raise exception 'labsite_members_missing'; end if;
  if jsonb_typeof(v_site.content->'pages')<>'object' then raise exception 'labsite_pages_missing'; end if;
  if jsonb_typeof(v_site.content->'theme')<>'object' then raise exception 'labsite_theme_missing'; end if;
  if coalesce(v_site.content->'theme'->>'background','')='' or coalesce(v_site.content->'theme'->>'surface','')='' or coalesce(v_site.content->'theme'->>'foreground','')='' or coalesce(v_site.content->'theme'->>'muted','')='' or coalesce(v_site.content->'theme'->>'accent','')='' then raise exception 'labsite_theme_incomplete'; end if;
  ...
  set state='final_review',updated_at=now(),summary=summary || jsonb_build_object('finalizedAt',now(),'portrait',v_portrait,'portraitGate','passed','projects',v_projects,'research',v_research,'publications',v_publications,'labSiteContract','complete')
  where id=p_run_id;
  ...
  values(p_run_id,'final_review','Engine v3 production passed deterministic review gates, including portrait and complete LabSite renderer contract.',jsonb_build_object('siteId',v_run.site_id,'portrait',v_portrait,'portraitGate','passed','labSiteContract','complete'));
