...
function payerMeta(payload: J) {
  ...
  return {
    name: [text(name.given_name, 200), text(name.surname, 200)].filter(Boolean).join(" "),
    email: text(payer.email_address, 320),
  };
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (action === "create_order") {
      ...
      const { data: purchase, error: insertError } = await admin
        ...
        .insert({
          ...
          status: "pending",
          provider: "paypal",
          provider_metadata: { source: "labnarrative.com/intelligence/buy", paypal_environment: paypalEnvironment() },
        })
        .select("*")
        .single();
      ...
      const created = await paypal("/v2/checkout/orders", {
        method: "POST",
        headers: { "PayPal-Request-Id": `intelligence-${purchase.id}-v1` },
        body: JSON.stringify({
          intent: "CAPTURE",
          purchase_units: [{
            reference_id: purchase.id,
            custom_id: purchase.id,
            description: `LabNarrative Intelligence - ${selected.name} - ${selected.products} product analyses`.slice(0, 127),
      ...
      const { error: bindError } = await admin.from("intelligence_package_purchases").update({
        ...
        provider_metadata: { ...asObject(purchase.provider_metadata), paypal_create_status: text(created.payload.status, 50), paypal_environment: paypalEnvironment() },
      }).eq("id", purchase.id);
    ...
    if (action === "capture") {
      ...
      if (purchase.status === "paid") return json({ ok: true, paid: true, captureId: purchase.provider_capture_id || "", purchaseId: purchase.id, packageName: purchase.package_name }, 200, origin);
      ...
      if (text(unit.custom_id, 100) !== purchase.id || Math.round(orderValue * 100) !== Math.round(Number(purchase.amount) * 100) || orderCurrency !== String(purchase.currency).toUpperCase()) {
        return json({ error: "Payment order does not match this Intelligence purchase." }, 409, origin);
      }

      if (text(order.payload.status, 50) !== "COMPLETED") {
        order = await paypal(`/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, {
          method: "POST",
          headers: { "PayPal-Request-Id": `intelligence-${purchase.id}-capture` },
          body: "{}",
        });
      }
      ...
      if (Math.round(capturedValue * 100) !== Math.round(Number(purchase.amount) * 100) || capturedCurrency !== String(purchase.currency).toUpperCase()) {
        return json({ error: "Captured payment amount does not match this Intelligence purchase." }, 409, origin);
      }
      const payer = payerMeta(order.payload);
      const now = new Date().toISOString();
      const { error: completeError } = await admin.from("intelligence_package_purchases").update({
        status: "paid",
        ...
        provider_metadata: {
          ...asObject(purchase.provider_metadata),
          paypal_order_status: text(order.payload.status, 50),
          paypal_capture_status: captureStatus,
          paypal_environment: paypalEnvironment(),
        },
      }).eq("id", purchase.id).neq("status", "paid");
      if (completeError) throw new Error(completeError.message);
      return json({ ok: true, paid: true, captureId, purchaseId: purchase.id, packageName: purchase.package_name, productCount: purchase.product_count, payerEmail: payer.email }, 200, origin);
    }
