...
function sec1ToPkcs8(sec1: Uint8Array) {
  ...
  const algorithm = der(0x30, concat(ecPublicKeyOid, p256Oid));
  const privateKey = der(0x04, sec1);
...
async function buildJwt(method: string, requestPath: string, credentials: CoinbaseCredentials) {
  const key = await importCoinbaseKey(credentials.keySecret);
  const now = Math.floor(Date.now() / 1000);
  const header = { alg: "ES256", typ: "JWT", kid: credentials.keyName, nonce: randomHex(16) };
  const payload = {
    iss: "cdp",
    nbf: now,
    exp: now + 120,
...
async function signedGet(requestPath: string, credentials: CoinbaseCredentials) {
  const token = await buildJwt("GET", requestPath, credentials);
...
async function portfolioSummary(credentials: CoinbaseCredentials, portfolioUuid?: string | null) {
  ...
  const requestPath = `/api/v3/brokerage/portfolios/${encodeURIComponent(uuid)}?currency=USD`;
  const payload = await signedGet(requestPath, credentials);
  ...
  })).filter((position) => position.asset && (position.totalUsd > 0 || position.totalCrypto > 0))
    .sort((a, b) => b.totalUsd - a.totalUsd);
  const availableUsd = spotPositions.reduce((sum, position) => sum + position.availableUsd, 0);
  return {
    portfolioUuid: uuid,
...
async function verifyAndPersist(admin: Db, userId: string, accountId: string, credentials: CoinbaseCredentials) {
  ...
  const fingerprint = (await sha256(credentials.keyName)).slice(0, 16);
